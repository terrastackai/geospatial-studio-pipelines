...
1. Copy gfm_logger, gfm_data_processing and orchestrate_wrapper to current directory where `push_to_geoserver.py` lives
...
```
c3_create_operator --repository quay.io/geospatial-studio --dockerfile_template_path Dockerfile.template --log_level DEBUG --version v0.1.0 --local_mode push_to_geoserver.py push_to_geoserver_helper_functions.py gfm_logger gfm_data_processing
```

3. Remove gfm_logger and gfm_data_processing from current directory
```
rm -r gfm_logger gfm_data_processing orchestrate_wrapper ./*.cwl ./*.job.yaml push_to_geoserver.yaml
```
