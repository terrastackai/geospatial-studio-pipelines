...
```bash
cp -r ../../general_libraries/orchestrate_wrapper .
# cp -r ../../general_libraries/gfm_logger .             # optional
# cp -r ../../general_libraries/gfm_data_processing .    # optional
```
...
```bash
c3_create_operator --repository quay.io/geospatial-studio --dockerfile_template_path Dockerfile.template --log_level DEBUG --version v0.1.0 --local_mode template_process.py {list any other files/folders to include in the image}
```
...
```bash
rm -r orchestrate_wrapper
rm ./*.cwl
rm ./*.job.yaml
# rm -r gfm_logger
# rm -r gfm_data_processing
```
