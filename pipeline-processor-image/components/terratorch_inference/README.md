...
```bash
c3_create_operator --repository quay.io/geospatial-studio --dockerfile_template_path Dockerfile.template --log_level DEBUG --version v0.1.0 --local_mode terratorch_inference.py gfm_logger gfm_data_processing terratorch_inference_utils.py
```
...
```bash
rm -r gfm_logger gfm_data_processing orchestrate_wrapper terratorch_inference.yaml ./*.cwl ./*.job.yaml
```
